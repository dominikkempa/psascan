...
uint64_t file_size(std::string fname) {
...
void read_file(unsigned char* &text, long &length, std::string fname) {
  ...
  length = std::ftell(f); // No length was given.
  std::rewind(f);
  text = new unsigned char[length + 10];
  if (!text) {
    fprintf(stderr, "Error: cannot allocate text.\n");
  ...
  size_t fread_ret = fread(text, sizeof(unsigned char), length, f);
...
template<typename value_type>
void read_objects_from_file(value_type* &tab, int &length, std::string fname) {
  FILE *f = open_file(fname, "r");
  fseek(f, 0, SEEK_END);
  length = ftell(f) / sizeof(value_type); // No length was given.
  rewind(f);
...
void read_bwt(unsigned char* &BWT, unsigned long &dollar, int &length,
    std::string fname) {
  int _n = (int)file_size(fname.c_str());
  length = _n - sizeof(dollar);
  FILE *f = open_file(fname, "r");
  BWT = new unsigned char[length + 10];
  if (!BWT) {
    fprintf(stderr, "Error: cannot allocate BWT.\n");
    exit(1);
  }
  int fread_ret = fread(&dollar, sizeof(dollar), 1, f);
  if (fread_ret != 1) {
    fprintf(stderr, "Error: fread in line %s of %s returned %d\n",
        STR(__LINE__), STR(__FILE__), fread_ret);
    exit(1);
  }
  fread_ret = fread(BWT, sizeof(unsigned char), length, f);
  if (fread_ret != length) {
    fprintf(stderr, "Error: fread in line %s of %s returned %d\n",
        STR(__LINE__), STR(__FILE__), fread_ret);
    exit(1);
