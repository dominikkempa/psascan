...
void test(unsigned char *seq, long length) {
  ...
  long start_pos = utils::random_long(0L, length);
  ...
  reader_type *f = new reader_type(m, start_pos, bufsize);

  unsigned char *result = new unsigned char[length];
  for (long i = start_pos; i < length; ++i)
    result[i] = f->read();

  delete f;
  delete m;  // this also delete all files from multifile

  if (!std::equal(seq + start_pos, seq + length, result + start_pos)) {
