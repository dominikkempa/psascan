...
void test(int *tab, long length) {
  ...
  // Write tab into file.
  vbyte_stream_writer<int> *writer = new vbyte_stream_writer<int>(filename);
  for (long j = 0; j < length; ++j) writer->write(tab[j]);
  delete writer;

  // Create the async stream reader.
  long bufsize = utils::random_long(1, 100);  // in bytes
  async_vbyte_stream_reader<int> *reader = new async_vbyte_stream_reader<int>(filename, bufsize);
