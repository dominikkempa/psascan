...
void test(
   ...
   const std::uint64_t tail_beg,
   const std::uint64_t text_length) {

  // Write text to disk.
  const std::string text_filename = "tempfile" + utils::random_string_hash();
  ...
  const std::uint64_t tail_length = text_length - tail_beg;
  const std::uint8_t * const tail = text + tail_beg;

  std::uint8_t * const gt_begin_reversed = new std::uint8_t[tail_length];
  ...
  for (std::uint64_t t = 0; t < n_threads; ++t) {
    ...
    const std::uint8_t * const pat = text + stream_block_beg;
    const std::uint64_t pat_length = text_length - stream_block_beg;
...
void test_random(
  ...
  for (std::uint64_t tc = 0, dbg = 0; tc < testcases; ++tc, ++dbg) {
    ...
    const std::uint64_t mid_length = utils::random_int64(0L, max_length);
    ...
    const std::uint64_t tail_beg    = block_end + mid_length;
    const std::uint64_t text_length =
