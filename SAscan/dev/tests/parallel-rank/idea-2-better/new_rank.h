...
struct rank4n {
  static const long k_sb_size_bits = 13;
  static const long k_block_size_bits = 9;
...
static void construction_step_4(rank4n &r, unsigned char *text, long range_beg,
  ...
  for (long block_id = range_beg; block_id < range_end; ++block_id) {
    ...
    bool isfreq[256] = {false};  // XXX optimize
    unsigned char freq_map[256]; // XXX optimize
    unsigned char rare_map[256];  // XXX optimize
