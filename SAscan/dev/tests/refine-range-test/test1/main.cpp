...
int lcp_compare_2(unsigned char *text, unsigned char *pat, long pat_length, long j, long &lcp) {
...
//  static const long balancing_constant = /*64L*/1L;
  ...
  while (low + 1 != high) {
    ...
    if (lcp_compare_2(text, pat, pat_length, block_beg + block_psa[mid], lcp) <= 0) {
      high = mid;
      rlcp = lcp;
    } else {
      low = mid;
...
void test(unsigned char *text, long text_length, long tail_length,
  ...
  for (long prefix_length = 0L; prefix_length <= tail_length; ++prefix_length) {
    ...
    for (long new_prefix_length = prefix_length; new_prefix_length <= tail_length; ++new_prefix_length) {
      ...
      refine_range(text, block_beg, block_psa, left, right,
        prefix_length, new_prefix_length, tail, computed_newleft, computed_newright);
...
int main(int, char **) {
  ...
  test_random(5000000,  10,     20);
  test_random(5000000,  10,    256);

  test_random(500000,  100,     5);
  test_random(500000,  100,    20);
