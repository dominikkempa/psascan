...
void test(unsigned char *text, long block_beg, long block_end, long tail_beg, long text_length) {
  ...
  long tail_length = text_length - tail_beg;
  ...
  for (long t = 0; t < n_threads; ++t) {
    long stream_block_beg = tail_beg + t * stream_max_block_size;

    unsigned char *pat = text + stream_block_beg;
    long pat_length = text_length - stream_block_beg;
