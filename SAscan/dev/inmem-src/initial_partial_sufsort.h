...
void initial_partial_sufsort(unsigned char *text, long text_length,
    bitvector* &gt, bwtsa_t<T> *bwtsa, long max_block_size, long /*max_threads*/) {
  ...
  long *temp_sa = (long *)malloc(text_length * sizeof(long));
  ...
  for (long i = 0; i < n_blocks; ++i) {
    ...
    // compute sa using divsufsort64
    threads[i] = new std::thread(run_divsufsort<long>, text + block_beg,
        temp_sa + block_beg, block_size);
  }
  ...
  fprintf(stderr, "  Converting from 64-bit to 40-bit integers: ");
  start = utils::wclock();
  for (long i = 0; i < text_length; ++i)
    bwtsa[i].sa = temp_sa[i];
  free(temp_sa);
