...
template<typename T>
void new_bwt_from_sa_into_dest_aux(T *tab, unsigned char *text, long beg, long end,
...
template<typename T>
long new_bwt_from_sa_into_dest(T *sa, unsigned char *text, long length,
  ...
  for (long i = 0; i < n_blocks; ++i) {
    ...
    threads[i] = new std::thread(new_bwt_from_sa_into_dest_aux<T>,
