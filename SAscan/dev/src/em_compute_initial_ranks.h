...
void em_compute_initial_ranks(
  ...
  for (int t = n_threads - 1; t >= 0; --t) {
    ...
    threads[t] = new std::thread(em_compute_single_initial_rank<saidx_t>,
        block, block_psa, block_beg, block_end, stream_block_end, text_length,
        max_lcp, text_filename, tail_gt_begin_reversed, std::ref(ranges[t]));

    max_lcp = stream_block_size;
  }
  ...
  if (nontrivial_range) {
    ...
    long last_stream_block_beg = block_end + (n_threads - 1) * stream_max_block_size;
    long last_stream_block_end = std::min(last_stream_block_beg + stream_max_block_size, text_length);
    long last_stream_block_size = last_stream_block_end - last_stream_block_beg;

    result[n_threads - 1] = ranges[n_threads - 1].first;
    if (ranges[n_threads - 1].first != ranges[n_threads - 1].second) {
      fprintf(stderr, "\nError: initial_ranges for the last starting point is not empty!\n");
      std::exit(EXIT_FAILURE);
    }

    long prev_stream_block_size = last_stream_block_size;
    long prev_rank = result[n_threads - 1];
    for (long t = n_threads - 2; t >= 0; --t) {
      long stream_block_beg = block_end + t * stream_max_block_size;
      long stream_block_end = std::min(stream_block_beg + stream_max_block_size, text_length);
      long stream_block_size = stream_block_end - stream_block_beg;
      ...
      while (left != right) {
        ...
        if ((long)block_psa[mid] + prev_stream_block_size >= block_length) {
          if (gt_reader->access(text_length - (stream_block_end + (block_length - (long)block_psa[mid])))) left = mid + 1;
          else right = mid; 
        } else {
          long j = (long)block_psa[mid] + prev_stream_block_size;
