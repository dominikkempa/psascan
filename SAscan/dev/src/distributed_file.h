...
//
// static const int n = (100 << 20);
// int *t = new int[n], i, S = 0;
// for (i=0; i<n; ++i) t[i] = 5*i;
//
// distributed_file<int> *f = new distributed_file<int>("example", 10 << 20);
...
// f->finish_reading();           // this fails if you don't read the whole file
// delete f;
//==============================================================================
...
struct distributed_file {
  ...
  distributed_file(const char *filename, long maxsize)
    ...
    if (!m_elems_per_file) {
      fprintf(stderr, "Error: maxsize = %ld is too small to hold objects "
  ...
  void initialize_reading(long bufsize = 4 << 20) {
    ...
    m_bufelems = (bufsize + sizeof(value_type) - 1) / sizeof(value_type);
    if (m_bufelems == 0) {
      fprintf(stderr, "Error: bufsize %ld is too small to hold objects "
          "of size %lu\n", bufsize, sizeof(value_type));
      std::exit(EXIT_FAILURE);
    }
    m_buf = new value_type[m_bufelems];
...
private:
  ...
  enum { STATE_INIT,    // right after creating (before init_writing)
         ...
         STATE_READ     // after finish_reading, waiting to die
  } m_state;
  ...
  long m_cur_file_written;  // current elems written to file
  long m_elems_per_file;  // max elems written to file
