...
void test(int *tab, long length) {
  ...
  long skip_elems = utils::random_int(0, length);
  async_backward_skip_stream_reader<int> *reader = new async_backward_skip_stream_reader<int>(filename, skip_elems, bufsize);

  // Read tab into filename using writer.
  int *tab_read = new int[length - skip_elems];
  for (long i = 0; i < length - skip_elems; ++i)
    tab_read[i] = reader->read();
  std::reverse(tab_read, tab_read + length - skip_elems);

  // Delete the reader.
  delete reader;

  if (!std::equal(tab, tab + length - skip_elems, tab_read)) {
    fprintf(stderr, "Error!\n");
    if (length < 100) {
      fprintf(stderr, "\tlength = %ld\n", length);
      fprintf(stderr, "\tskip_elems = %ld\n", skip_elems);
