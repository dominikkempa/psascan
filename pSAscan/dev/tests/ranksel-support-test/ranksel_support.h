...
struct ranksel_support {
  ...
  ranksel_support(bitvector *bv, long length, long max_threads) {
    ...
    m_chunk_size = std::min(/*(1L << 20)*/3L, (m_length + max_threads - 1) / max_threads);
    ...
    // Only for testing //
    m_ones = ones;
    long j = (m_length / m_chunk_size) * m_chunk_size;;
    while (j < m_length) m_ones += bv->get(j++);
  }
  ...
  inline long select0(long i) const {
    if (i < 0 || i >= (m_length - m_ones)) {
      fprintf(stderr, "Error: select0 arg (%ld) out of bounds [0..%ld)\n", i, m_length - m_ones);
  ...
  inline long select1(long i) const {
    if (i < 0 || i >= m_ones) {
      fprintf(stderr, "Error: select1 arg (%ld) out of bounds [0..%ld)\n", i, m_ones);
      std::exit(EXIT_FAILURE);
    }
  ...
  inline long rank(long i) const {
    if (i < 0 || i > m_length) {
      fprintf(stderr, "Error: rank arg (%ld) out of bounds [0..%ld]\n", i, m_length);
      std::exit(EXIT_FAILURE);
    }
  ...
  inline long rank0(long i) const {
    if (i < 0 || i > m_length) {
      fprintf(stderr, "Error: rank0 arg (%ld) out of bounds [0..%ld]\n", i, m_length);
      std::exit(EXIT_FAILURE);
    }
