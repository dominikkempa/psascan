...
template<unsigned isa_sampling_rate_log = 8>
struct sparse_isa {
  ...
  static void compute_sparse_isa_aux(const long *psa, long block_beg,
  ...
  sparse_isa(long *psa, unsigned char *text, long length, long i0, rank4n<> *rank, long max_threads) {
    m_psa = psa;
    m_length = length;
    m_rank = rank;
    m_text = text;
    ...
    for (long t = 0; t < n_blocks; ++t) {
      ...
      threads[t] = new std::thread(compute_sparse_isa_aux, m_psa, block_beg,
          block_end, m_length, m_sparse_isa, std::ref(m_last_isa));
    }
