...
void test(T *tab, long length, long max_threads) {
  ...
  start = utils::wclock();
  S *computed = parallel_expand<T, S>(tab, length, max_threads);
  fprintf(stderr, "%.2Lf\n", utils::wclock() - start);
...
void test_random(long length) {
  ...
  uint40 *tab2 = (uint40 *)tab;

  fprintf(stderr, "Generating input: ");
  long double start = utils::wclock();
  long initial_chunk = std::min(1L << 20, length);
  for (long i = 0; i < initial_chunk; ++i)
    tab2[i] = uint40(utils::random_long(1L, 137438953472L));
  for (long i = initial_chunk; i < length; ++i)
    tab2[i] = tab2[i - initial_chunk];
  fprintf(stderr, "%.2Lf\n", utils::wclock() - start);

  // Run the test on generated input.
  test<uint40, long>(tab2, length, 24);
  test<uint40, long>(tab2, length, 12);
  test<uint40, long>(tab2, length, 6);
  test<uint40, long>(tab2, length, 4);
  test<uint40, long>(tab2, length, 1);
