...
void test(unsigned char *text, long text_length, long left_block_beg, long left_block_size,
  ...
  // Compute the gap array of the left half-block wrt the tail.
  long *left_block_correct_gap = new long[left_block_size + 1];
  gap_ptr = 0L;
  gap_val = 0L;
  for (long j = 0; j < text_length; ++j) {
    if (left_block_beg <= text_sa[j] && text_sa[j] < left_block_end) {
      left_block_correct_gap[gap_ptr++] = gap_val;
      gap_val = 0L;
    } else if (left_block_end <= text_sa[j])
      ++gap_val;
  }
  left_block_correct_gap[gap_ptr++] = gap_val;
  ...
  compute_left_gap(left_block_size, right_block_size, block_gap_2n, left_gap_bv, out_filename, max_threads, ram_budget);

  // Now, read compute gap array from out_filename (encoded using v-byte encoding)
  // and compare to right_block_gap.
  long *left_block_computed_gap = new long[left_block_size + 1];
  vbyte_stream_reader *reader = new vbyte_stream_reader(out_filename);
  for (long j = 0; j < left_block_size + 1; ++j)
    left_block_computed_gap[j] = reader->read();
  delete reader;
  delete left_gap_bv;
  delete block_gap_2n;
  delete text_sa;
  ...
  if (!std::equal(left_block_computed_gap, left_block_computed_gap + left_block_size + 1, left_block_correct_gap)) {
    ...
    fprintf(stdout, "\tcorrect left block gap: ");
    for (long j = 0; j < left_block_size + 1; ++j)
      fprintf(stdout, "%ld ", left_block_correct_gap[j]);
    fprintf(stdout, "\n");
    fprintf(stdout, "\tcomputed left block gap: ");
    for (long j = 0; j < left_block_size + 1; ++j)
      fprintf(stdout, "%ld ", left_block_computed_gap[j]);
    fprintf(stdout, "\n");
    fflush(stdout);
    std::exit(EXIT_FAILURE);
  }

  delete[] left_block_computed_gap;
  delete[] left_block_correct_gap;
}
