...
template<long k_cblock_size_bits = 16, long k_sigma_bits = 8>
struct rank4n {
  ...
  inline long rank(long i, unsigned char c, long &dist, long &scan_length) {
    if (i <= 0) return 0L;
    ...
    if (end < endmax) {
      dist = std::labs(idx - end);
      scan_length = 1;
    }
    while (end < endmax && (m_trunk[cblock_beg + end] >> (k_cblock_size_bits + 1)) < block_local_i) {
      ++scan_length;
      ++end;
    }
