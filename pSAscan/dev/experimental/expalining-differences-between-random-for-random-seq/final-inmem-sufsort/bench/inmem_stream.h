...
void inmem_parallel_stream(
  ...
  while (j > stream_block_beg) {
    ...
    if (!compute_gt_out) {  // Version that does not compute gt_out.
      for (long t = 0; t < b->m_filled; ++t, --j) {
        ...
        i = (block_offset_type)(count[c] + rank->rank((long)(i - (i > i0)), c, dist, scan_length));


        ++n_scans;
        total_scan_length += scan_length;
        total_dist += dist;
  ...
  fprintf(stderr, "\nn_scans = %ld, total_dist = %ld, total_scan_len = %ld\n",
      n_scans, total_dist, total_scan_length);
  fprintf(stderr, "avg_dist = %.4Lf, avg_scan_len = %.4Lf\n",
      (long double)total_dist / n_scans, (long double)total_scan_length / n_scans);
}
